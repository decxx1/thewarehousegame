...
export class Engine {
  ...
  /**
   * Parse a standard Sokoban level string into game state.
   * Characters: # wall, space floor/void, . target, $ box, * box+target, @ player, + player+target
   */
  parseLevel(str) {
      ...
      for (let x = 0; x < w; x++) {
        ...
        switch (ch) {
          ...
          case '+':
            ...
            break;
          default:
            grid[y][x] = -1; // void/space
            break;
        }
      }
    }

    // Flood fill from player to distinguish floor (-1 outside) from reachable spaces
    if (player) {
      this._floodFillFloor(grid, w, h, player.x, player.y, boxes);
    }

    return {
      grid, w, h, targets, boxes, player,
      moves: 0,
      pushes: 0,
      history: [] // [{px,py,bi?,bx?,by?}]
    };
  }

  /**
   * Flood fill from player position to mark reachable void cells as floor.
   * This handles levels where internal spaces are represented as ' '.
   */
  ...
  /**
   * Load a level string and set as current state.
   */
  loadLevel(levelStr) {
  ...
  /**
   * Check if all targets have boxes on them (win condition).
   */
  checkWin() {
    const s = this.state;
    if (!s) return false;
  ...
  /**
   * Reset the current level (reload from string).
   */
  resetLevel(levelStr) {
    return this.loadLevel(levelStr);
  }
}
